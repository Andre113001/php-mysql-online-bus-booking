...
if($_SESSION['booking_id'] == false) {
    ...
    $result = mysqli_fetch_assoc($sql) or die("Data Retrieval Failed.");
    ...
    $passengers_array = explode(',', $result['booking_passengers']);

    $discounted_passengers = explode(',', $fetch_discount['discount_passengerNames']);
    $discount_counter = 0;
    $actualPrice = 0;

    foreach ($passengers_array as $passenger) {
        if (in_array($passenger, $discounted_passengers)) {
            $discount = $farePrice * 0.2;
            $actualPrice += $farePrice - $discount; ?> 
    
            <script>console.log('<?php echo $passenger;?>')</script>
    ...
    // if ($discount_counter > 0)
    // {
    //     if ($fetch_discount['discount_type'] == NULL) {
    //         $type = 'NONE';
    //     } else {
    //         $reveal_discount = '';
    //         $type = $fetch_discount['discount_type'];
    ...
    <div class="container-fluid container-sm d-flex align-items-center justify-content-between brand">
        ...
        <nav id="navbar" class="navbar">
            <div class="lc-block d-grid gap-2 d-md-flex justify-content-md-start">
                ...
                <a href="profile-ticket.php?exit=true"><button class="hero-btn rounded">Exit</button></a>
            </div>
        </nav> 
        <!-- END nav -->
    </div>
          ...
          <div class="card border-0" style="border-radius: 1rem;" id="content">
            ...
            <div class="row container mb-2">
                ...
                <div class="col m-2">
                    ...
                    <span><?php echo $_SESSION['customer_contactnum']; ?></span>
                </div>
            </div>
            ...
            <div class="row container mb-5">
                ...
                <div class="col m-2">
                    <h5>Gender: </h5>
                    <span><?php echo $_SESSION['customer_gender']; ?></span>
                </div>
                <div class="col m-2">
                    <h5>Discount: </h5>
